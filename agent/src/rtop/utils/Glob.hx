...
class Glob
{
  ...
  {
    return regex.match(normalizePath(path));
  }
  ...
  {
    return unsafePartial(normalizePath(path));
  }
  ...
  {
    return unsafeMatch(normalizePath(path));
  }
  ...
  public static function normalizePath(path:String) {
    var isAbsolute = path.charCodeAt(0) == '/'.code,
        trailingSlash = path.charCodeAt(path.length-1) == '/'.code;
    path = normalizeArray([ for (part in path.split('/')) if (part != null && part != '') part ], !isAbsolute).join('/');
    if (isAbsolute) {
      path = '/' + path;
    }
  ...
  static function normalizeArray(parts:Array<String>, allowAboveRoot:Bool)
  {
